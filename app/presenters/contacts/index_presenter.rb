module Contacts
  class IndexPresenter < ApplicationPresenter
    ...
    def consultations
      @consultations ||= filter.present? ? filter.includes(:contact, :action).distinct.order('consultations.created_at DESC').reverse : Consultation.none
    end
    ...
    def scope_by_date
      scope = user_scope.where(district_id: current_user.panchayat.district.id, created_at: date_window)
      assigned_to_me == 'Assigned to Me' ? scope.where(doctor_id: current_user.id) : scope
    end
